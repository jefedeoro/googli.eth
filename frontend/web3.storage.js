...
(() => { var __webpack_modules__ = { 9824: (t, e) => { "use strict"; const r = "undefined" != typeof BigUint64Array,
        ...
        (() => { "use strict";
                ...
                class $ { constructor({ cid: t, bytes: e, value: r }) { if (!t || !e || void 0 === r) throw new Error("Missing required argument");
                    ...
                    get(t = "/") { return ((t, e) => { let r = t; for (const [t, n] of e.entries()) { if (r = r[n], null == r) throw new Error(`Object has no property at ${e.slice(0,t+1).map((t=>`[${JSON.stringify(t)}]`)).join("")}`);const o=T.k0.asCID(r);if(o)return{value:o,remaining:e.slice(t+1).join("/")}}return{value:r}})(this.value,t.split("/").filter(Boolean))}}var q=__webpack_require__(6945),H=__webpack_require__(3925);class V{constructor(t,e,r={}){if("number"!=typeof e||e<=0)throw new Error("invalid target chunk size");this._reader=t,this._targetSize=e,this._decoders=[H,q,c,...r.decoders||[]]}async*cars(){const t=await this._reader.getRoots();if(1!==t.length)throw new Error(`unexpected number of roots: ${t.length}`);let e;for await(const r of this._cars(t[0]))e=r.channel,r.out&&(yield r.out);if(!e)throw new Error("missing CAR writer channel");e.writer.close(),yield e.out}async _get(t){const e=await this._reader.get(t);if(!e)throw new Error(`missing block for ${t}`);const{bytes:r}=e,n=this._decoders.find((e=>e.code===t.code));if(!n)throw new Error(`missing decoder for ${t.code}`);return new $({cid:t,bytes:r,value:n.decode(r)})}async*_cars(t,e=[],r){const n=await this._get(t);if((r=r||Object.assign(A.create(t),{size:0})).size>0&&r.size+n.bytes.byteLength>=this._targetSize){r.writer.close();const{out:t}=r;r=function(t){const e=Object.assign(A.create(t[0].cid),{size:t.reduce(((t,e)=>t+e.bytes.byteLength),0)});for(const r of t)e.writer.put(r);return e}(e),yield{channel:r,out:t}}e=e.concat(n),r.size+=n.bytes.byteLength,r.writer.put(n);for(const[,t]of n.links())for await(const n of this._cars(t,e,r))r=n.channel,yield n;if(!r)throw new Error("missing CAR writer channel");yield{channel:r}}static async fromIterable(t,e,r){const n=await b.fromIterable(t);return new V(n,e,r)}static async fromBlob(t,e,r){const n=await t.arrayBuffer(),o=await b.fromBytes(new Uint8Array(n));return new V(o,e,r)}}const G=globalThis.fetch,W=(globalThis.Request,globalThis.Response,globalThis.Blob),J=globalThis.File,K=O;class X{constructor({token:t,endpoint:e=new URL("https://api.web3.storage")}){this.token=t,this.endpoint=e}static headers(t){if(!t)throw new Error("missing token");return{Authorization:`Bearer ${t}`,"X-Client":"web3.storage"}}static async put({endpoint:t,token:e},r,{onRootCidReady:i,onStoredChunk:s,maxRetries:a=5,wrapWithDirectory:c=!0,name:u}={}){const h=new K;try{const{out:f,root:l}=await async function({input:t,blockstore:e,hasher:r,maxChunkSize:i,maxChildrenPerNode:s,wrapWithDirectory:a}){if(!t||Array.isArray(t)&&!t.length)throw new Error("missing input file(s)");const c=e||new O,u=await n(o(I(t),(t=>(0,S.importer)(t,c,{...U,hasher:r||U.hasher,maxChunkSize:i||U.maxChunkSize,maxChildrenPerNode:s||U.maxChildrenPerNode,wrapWithDirectory:!1!==a&&U.wrapWithDirectory}))));if(!u||!u.cid)throw new Error("given input could not be parsed correctly");const h=u.cid,{writer:f,out:l}=await A.create([h]),p=l[Symbol.asyncIterator]();let d;return{root:h,out:{[Symbol.asyncIterator](){if(null!=d)throw new Error("Multiple iterator not supported");return d=(async()=>{for await(const t of c.blocks())await f.put(t);await f.close(),e||await c.close()})(),{async next(){const t=await p.next();return t.done&&await d,t}}}}}}({input:Array.from(r).map((t=>({path:t.name,content:t.stream()}))),blockstore:h,wrapWithDirectory:c,maxChunkSize:1048576,maxChildrenPerNode:1024});i&&i(l.toString());const p=await b.fromIterable(f);return await X.putCar({endpoint:t,token:e},p,{onStoredChunk:s,maxRetries:a,name:u})}finally{await h.close()}}static async putCar({endpoint:t,token:n},o,{name:i,onStoredChunk:s,maxRetries:a=5}={}){const c=new URL("/car",t);let u=X.headers(n);i&&(u={...u,"X-Name":i});const h=await o.getRoots();if(null==h[0])throw new Error("missing root CID");if(h.length>1)throw new Error("too many roots");const f=h[0].toString(),l=new V(o,10485760),p=e(3,(async t=>{const e=[];for await(const r of t)e.push(r);const n=new W(e,{type:"application/car"}),o=await r((async()=>{const t=await G(c.toString(),{method:"POST",headers:u,body:n}),e=await t.json();if(!t.ok)throw new Error(e.message);if(e.cid!==f)throw new Error(`root CID mismatch, expected: ${f}, received: ${e.cid}`);return e.cid}),{retries:a});return s&&s(n.size),o}));for await(const t of p(l.cars()));return f}static async get({endpoint:t,token:e},r){const n=new URL(`/car/${r}`,t);return function(t){const e=Object.assign(t,{unixFsIterator:async function*(){if(!t.ok)throw new Error(`Response was not ok: ${t.status} ${t.statusText} - Check for { "ok": false } on the Response object before calling .unixFsIterator`);if(!t.body)throw new Error("No body on response");const e=new K;try{for await(const r of async function*(t,{roots:e,blockstore:r}={}){const n=await _.fromIterable(function(t){return Symbol.asyncIterator in t?t:R(t)}(t)),o=r||new O;for await(const t of n)await o.put(t.cid,t.bytes);const i=j.fromBlockstore(o);e&&0!==e.length||(e=await n.getRoots());for(const t of e)yield*L.recursive(t,i)}(t.body,{blockstore:e}))yield r}finally{await e.close()}},files:async()=>{if(!t.ok)throw new Error(`Response was not ok: ${t.status} ${t.statusText} - Check for { "ok": false } on the Response object before calling .files`);const r=[];for await(const t of e.unixFsIterator()){if("directory"===t.type)continue;const e=await Y(t);r.push(e)}return r}});return e}(await G(n.toString(),{method:"GET",headers:X.headers(e)}))}static async delete({endpoint:t,token:e},r){throw console.log("Not deleting",r,t,e),Error(".delete not implemented yet")}static async status({endpoint:t,token:e},r){const n=new URL(`/status/${r}`,t),o=await G(n.toString(),{method:"GET",headers:X.headers(e)});if(404!==o.status){if(!o.ok)throw new Error(o.statusText);return o.json()}}static async*list(t,{before:e=(new Date).toISOString(),maxResults:r=1/0}={}){let n=0;const o=r>100?100:r;for await(const i of async function*(t,e,r){let n=await t(e,r);yield n;let o=D(n.headers.get("Link")||"");for(;o&&o.next;)n=await t(e,o.next),yield n,o=D(n.headers.get("Link")||"")}((function({endpoint:t,token:e},{before:r,size:n}){const o=new URLSearchParams({before:r,size:n.toString()}),i=new URL(`/user/uploads?${o}`,t);return G(i.toString(),{method:"GET",headers:{...X.headers(e),"Access-Control-Request-Headers":"Link"}})}),t,{before:e,size:o})){if(!i.ok)throw new Error(`${i.status} ${i.statusText}`);const t=await i.json();for(const e of t){if(++n>r)return;yield e}}}put(t,e){return X.put(this,t,e)}putCar(t,e){return X.putCar(this,t,e)}get(t){return X.get(this,t)}delete(t){return X.delete(this,t)}status(t){return X.status(this,t)}list(t){return X.list(this,t)}}async function Y({content:t,path:e,cid:r}){const n=[];for await(const e of t())n.push(e);const o=new J(n,function(t){const e=t.indexOf("/");return-1===e?t:t.substring(e+1)}(e));return Object.assign(o,{cid:r.toString()})}const Z=document.querySelector("#upload-form"),Q=document.querySelector("#filepicker");function tt(t){console.log(t)}document.querySelector("#output"),tt("> ⁂ waiting for form submission..."),Z.addEventListener("submit",(async function(t){t.preventDefault(),tt("> 📦 creating web3.storage client");const e=new X({token:"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDQyQjZkN0QxNDM4ZjlhMTkyMTk5OEQ5MjRmMzA2OTg4YmRkNjNFNTgiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2Mjg1NDU1ODkxNzIsIm5hbWUiOiJnb29nbGkifQ.FhkQFDCRBuPe8Yp964QG8e4DV6-T3BlS7dpbBzbrh-Q"});tt("> 🤖 chunking and hashing the files (in your browser!) to calculate the Content ID");const r=Q.files,n=await e.put(r,{onRootCidReady:t=>{tt(`> 🔑 locally calculated Content ID: ${t} `),tt("> 📡 sending files to web3.storage ")},onStoredChunk:t=>tt(`> 🛰 sent ${t.toLocaleString()} bytes to web3.storage`)});var o;tt(`> ✅ web3.storage now hosting ${n}`),o=`https://dweb.link/ipfs/${n}`,console.log(`> 🔗 ${o}`),tt("> 📡 fetching the list of all unique uploads on this account");let i=0;for await(const t of e.list())tt(`> 📄 ${t.cid}  ${t.name}`),i+=t.dagSize||0;tt(`> ⁂ ${i.toLocaleString()} bytes stored!`)}),!1)})()})();